...
void RenderDebugTab()
{
    ...
    switch (Gw2Fetcher::GetStatus())
    {
        ...
        case Gw2Fetcher::Gw2Status::Connecting: gw2StatusText = Localization::GetText("status_connecting"); break;
    ...
    if (ImGui::Button("Export Logs"))
    {
        ...
        if (addonDir)
        {
            std::string logPath = std::string(addonDir) + "\\drf_logs.txt";
            ...
            if (logFile.is_open())
            {
                for (auto& log : logs)
                {
                    ...
                    logFile << "[" << timeStr << "] [" << log.type << "] " << log.message << "\n";
                }
                logFile.close();
                DrfClient::Log("DRF logs exported to: " + logPath, "info");
            }
        }
    }
    if (ImGui::IsItemHovered())
        ImGui::SetTooltip("Export DRF logs to a text file");
    ImGui::SameLine();
    ...
    if (ImGui::Button("Export Logs"))
    {
        ...
        if (addonDir)
        {
            std::string logPath = std::string(addonDir) + "\\gw2_api_logs.txt";
            std::ofstream logFile(logPath);
            if (logFile.is_open())
            {
                for (auto& log : logs)
                ...
                logFile.close();
                DrfClient::Log("GW2 API logs exported to: " + logPath, "info");
            }
        }
    }
    if (ImGui::IsItemHovered())
        ImGui::SetTooltip("Export GW2 API logs to a text file");
    ImGui::SameLine();
    ImGui::Text("%s", Localization::GetText("last_100_entries"));
    ...
    const char* apiKeyStatus = SettingsManager::GetCurrentGw2ApiKey().empty() ? Localization::GetText("not_set") : Localization::GetText("set");
    snprintf(apiKeyLabel, sizeof(apiKeyLabel), "API Key: %s", apiKeyStatus);
    ImGui::Text("%s", apiKeyLabel);
    if (ImGui::IsItemHovered())
        ImGui::SetTooltip("%s", Localization::GetText("api_key_tooltip"));
    char drfTokenLabel[256];
    const char* drfTokenStatus = SettingsManager::GetCurrentDrfToken().empty() ? Localization::GetText("not_set") : Localization::GetText("set");
    snprintf(drfTokenLabel, sizeof(drfTokenLabel), "DRF Token: %s", drfTokenStatus);
