...
void AutoReset::Tick()
{
    ...
    {
        static bool sessionCompleteTriggered = false;
        auto duration = ItemTracker::GetSessionDuration();
        auto targetSeconds = static_cast<long long>(g_Settings.sessionCompleteHours) * 3600;
        
        if (duration.count() >= targetSeconds)
        {
            if (!sessionCompleteTriggered)
            {
                char msg[256];
                snprintf(msg, sizeof(msg), Localization::GetText("session_complete_msg"), g_Settings.sessionCompleteHours);
                UINotifications::AddGenericNotification(Localization::GetText("session_complete_title"), msg, "", "Rare", true);
                sessionCompleteTriggered = true;
            }
        }
        else
        {
            sessionCompleteTriggered = false;
        }
    }
    ...
    {
        static bool resetWarningTriggered = false;
        auto warningTime = next - std::chrono::minutes(g_Settings.resetWarningMinutes);
        if (now >= warningTime && now < next)
        {
            if (!resetWarningTriggered)
            {
                char msg[256];
                snprintf(msg, sizeof(msg), Localization::GetText("reset_warning_msg"), g_Settings.resetWarningMinutes);
                UINotifications::AddGenericNotification(Localization::GetText("reset_warning_title"), msg, "", "Rare", true);
                resetWarningTriggered = true;
            }
        }
        else if (now < warningTime)
        {
            resetWarningTriggered = false;
        }
    }
